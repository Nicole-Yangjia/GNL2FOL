...
class ModelEvaluator:
    def __init__(self, model_path, model=None, tokenizer=None, base_model_name="meta-llama/Llama-3.1-8B-Instruct"):
    ...
    def load_model(self):
        ...
        try:
            base_model = AutoModelForCausalLM.from_pretrained(
                self.model_path.split('/')[-1] if 'cot_generation_model' in self.model_path else self.model_path,
            ...
            if 'cot_generation_model' in self.model_path:
                print("加载LoRA权重...")
                try:
                    self.model = PeftModel.from_pretrained(base_model, self.model_path)
                except:
                    print("无法加载LoRA权重，使用基础模型")
...
def evaluate_all_data(model_path, data_path, num_samples=None):
    ...
    evaluator = ModelEvaluator(model_path, base_model_name="meta-llama/Llama-3.1-8B-Instruct")
    ...
    with open("full_evaluation_results.json", "w", encoding="utf-8") as f:
        simplified_results = []
        for r in results:
            simplified = {
...
if __name__ == "__main__":
    MODEL_PATH = "./cot_generation_model_0107"
