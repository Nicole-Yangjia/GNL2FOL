...
class FixedLogicParser:
    ...
    def _parse_atomic(self):
        if self.current_pos + 3 <= len(self.input_str) and self.input_str[
                                                           self.current_pos:self.current_pos + 3] == 'leq':
            self.current_pos += 3
            self._next_token()
...
class QwenModelEvaluator:
    def __init__(self, model_path, base_model_path=None):
        ...
        self.base_model_path = base_model_path or "/root/sj-tmp/llama_finetuning/Qwen2.5-7B-Instruct"
        self.model = None
        self.tokenizer = None
    ...
    def load_model(self):
        print(f"加载Qwen模型从: {self.model_path}")
        try:
            ...
            print(f"无法从 {self.model_path} 加载tokenizer，尝试从基础模型加载")
            try:
                self.tokenizer = AutoTokenizer.from_pretrained(
                ...
                raise Exception(f"无法加载tokenizer: {e2}")
        ...
        try:
            print(f"加载基础模型从: {self.base_model_path}")
            ...
            print(f"检查是否为LoRA微调模型: {self.model_path}")
            try:
                self.model = PeftModel.from_pretrained(base_model, self.model_path)
                print("成功加载LoRA微调权重")
            except Exception as e:
                print(f"无法加载LoRA权重: {e}，使用基础模型")
    ...
    def evaluate_sample(self, sample):
        try:
            ...
            result = {
                ...
                'generated_cot': extracted['cot'][:500] + '...' if extracted['cot'] and len(extracted['cot']) > 500 else
                extracted['cot'],
    ...
    def print_evaluation_summary(self, results):
        ...
        print(
            f"  logic正确但FOL不一致但有效: {logic_correct_fol_inconsistent_but_valid}/{total} ({logic_correct_fol_inconsistent_but_valid / total:.2%})")
        print(
            f"  logic正确但FOL不一致且无效: {logic_correct_fol_inconsistent_and_invalid}/{total} ({logic_correct_fol_inconsistent_and_invalid / total:.2%})")
        print(f"\nFOL有效性统计:")
        ...
        if len(types) > 1 or 'unknown' not in types:
            ...
            for type_name in sorted(types):
                ...
                print(
                    f"    logic_output准确率: {type_logic_correct}/{type_count} ({type_logic_correct / type_count:.2%})")
...
def evaluate_qwen_model(model_path, data_path, num_samples=None, use_all_data=False):
    ...
    if use_all_data:
        ...
        print(f"使用所有数据评估: {len(test_samples)} 个样本")
    else:
        print("\n2. 划分验证集...")
        ...
        print(f"验证集大小: {len(test_samples)} 个样本")
    if num_samples and num_samples < len(test_samples):
        test_samples = test_samples[:num_samples]
        print(f"评估样本数: {len(test_samples)} 个样本")
    base_model_path = "/root/sj-tmp/llama_finetuning/Qwen2.5-7B-Instruct"
    evaluator = QwenModelEvaluator(model_path, base_model_path=base_model_path)
    model, tokenizer = evaluator.load_model()
    print(f"\n4. 评估模型 ({len(test_samples)} 个样本)...")
    results, errors = evaluator.evaluate_batch(test_samples)
    if errors:
    ...
    output_file = f"qwen_evaluation_results_{timestamp}.json"
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for r in results:
            simplified = {
                ...
                'generated_preview': r.get('generated_preview', ''),
            }
...
if __name__ == "__main__":
    MODEL_PATH = "./qwen_cot_generation_model"
    DATA_PATH = "game_data_two4.xlsx"
    print("Qwen模型评估配置:")
    print(f"模型路径: {MODEL_PATH}")
    print(f"数据路径: {DATA_PATH}")
    print("\n开始评估整个数据集...")
    summary = evaluate_qwen_model(MODEL_PATH, DATA_PATH, num_samples=None, use_all_data=True)
    if summary:
        print(f"\n评估完成!")
        print(f"logic_output准确率: {summary['logic_accuracy']:.2%}")
        print(f"FOL准确率: {summary['fol_accuracy']:.2%}")
        print(f"同时正确率: {summary['both_accuracy']:.2%}")
    else:
        print("\n评估失败!")