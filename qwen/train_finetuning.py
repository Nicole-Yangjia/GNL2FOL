...
{output_text}<|im_end|>"""
    ...
    def initialize_model(self):
        ...
        try:
            ...
            print(f"使用flash attention失败，尝试不使用量化...")
            self.model = AutoModelForCausalLM.from_pretrained(
                self.model_path,
                device_map="auto",
                dtype=torch.float16,
...
def main():
    ...
    # 配置
    MODEL_PATH = "/root/sj-tmp/llama_finetuning/Qwen2.5-7B-Instruct"
    DATA_PATH = "game_data_two3.xlsx"
    OUTPUT_DIR = "./qwen_cot_generation_model"
    ...
    try:
        ...
        if training_samples:
            ...
            print(f"\n输入示例: {test_sample['input'][:200]}...")

            # 手动构建Qwen2.5格式进行测试
            test_input = f"""<|im_start|>user
{test_sample['input']}<|im_end|>
<|im_start|>assistant"""
